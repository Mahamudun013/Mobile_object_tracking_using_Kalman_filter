...
for i = 1:length(frame_list) 
    ...
    threshold_image = (gaussian_image < -30);
    ...
    object_image = zeros(size(threshold_image)); 
    object_image(Track_data(i,1),Track_data(i,2)) = 1;
    %figure, imshow(object_image);

    object_image = imdilate(object_image, SE);
    subplot(224);
    imagesc(threshold_image + object_image);
    title('thresholded and detect centroid');
